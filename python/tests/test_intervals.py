...
from trafficrs.intervals import Interval, IntervalCollection
...
class TestIntervalAdd:
    def test_partialsup(self) -> None:
        assert i1 + i4 == IntervalCollection(start=[h0], stop=[h5])

    def test_totalsup(self) -> None:
        assert i2 + i5 == IntervalCollection(start=[h0], stop=[h8])
...
class TestCollection:
    ...
    def test_consolidate(self) -> None:
        assert c1 == IntervalCollection(h0, h5)
        assert c2 == IntervalCollection(h0, h5)
        assert c3 == IntervalCollection(h0, h8)
        assert c5 == c5
        assert c6 == c6

        res = IntervalCollection(i1, i2, i3, i4)
...
class TestCollectionAdd:
    ...
    def test_diffsup(self) -> None:
        assert c1 + c4 == IntervalCollection(start=[h0], stop=[h8])

    def test_diffnosup(self) -> None:
        assert c1 + c7 == IntervalCollection([h0, h7], [h5, h8])
